<div class="space-y-6">
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h3 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $produto->nome }}</h3>
                <p class="text-gray-600">Produto ID: <span class="font-medium text-gray-900">#{{ $produto->id }}</span></p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3">
                <button onclick="editOpenedFromShow = true; closeShowModal(); openEditModal({{ $produto->id }});" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium transition-all duration-200 shadow-md hover:shadow-lg">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                    </svg>
                    Editar
                </button>
                <button onclick="deleteFromModal({{ $produto->id }})" class="inline-flex items-center justify-center px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium transition-all duration-200 shadow-md hover:shadow-lg">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
                ...
                <div class="p-6 max-h-67 overflow-y-auto">
                    <p class="text-gray-700 leading-relaxed text-justify break-words whitespace-pre-wrap">{{ $produto->descricao }}</p>
                </div>
            </div>
        </div>
